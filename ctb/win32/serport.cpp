...
int wxSerialPort::GetLineState(wxSerialLineState* flags)
{
    ...
    *(int*)flags = 0;
    if(GetCommModemStatus(fd,&stat)) {
	   if(stat & MS_CTS_ON)
		  *(int*)flags |= wxSERIAL_LINESTATE_CTS;
	   if(stat & MS_DSR_ON)
		  *(int*)flags |= wxSERIAL_LINESTATE_DSR;
	   if(stat & MS_RING_ON)
		  *(int*)flags |= wxSERIAL_LINESTATE_RING;
	   if(stat & MS_RLSD_ON)
		  *(int*)flags |= wxSERIAL_LINESTATE_DCD;
	   ok = true;
    }
    if(!ok) return -1;
    return 0;
